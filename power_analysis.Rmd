...
for(t in 1:12) {
  simd[,paste0("trial.",t)] = rlnorm(n = nrow(simd), 
                   meanlog = log((10^2) / sqrt(0.5^2 + 10^2)), 
                   sdlog = sqrt(log(1 + (0.5^2 / 10^2))))} #say raw LT mean = 10, raw LT SD = 1
...
effect_size = .3 # .25 in meta-analysis
...
fixef(m_simple)["trial_num"] <- .1
powerSim(m_simple, test = fixed("trial_type"), nsim =100) #we need to specify the fixed effect that we want to do, cause simrR can only calculate power for one particular coefficient each time
```
