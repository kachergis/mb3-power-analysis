...
generate_dataset <- function(n_labs=20, n_per_lab=16, effect_sizes=list(type = .3, age = 0, "age*type"=0)) {
...
fit_simple_model <- function(siml) {
  m1 <- lmer(looking_time ~ 1 + trial_type * trial_num + (1 | subjID), data=siml)
  return(summary(m1)$coefficients["trial_typesame","Pr(>|t|)"]) # "Estimate","t value",
}

# check both
fit_model <- function(siml) {
  m1 <- lmer(looking_time ~ 1 + trial_type * trial_num + trial_type * age + (1 | subjID) + (1 | labID), data=siml)
