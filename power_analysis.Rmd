...
generate_dataset <- function(n_labs=20, n_per_lab=16, effect_size=.3) {
  ...
  # DO WE NEED TO USE LOG-NORMAL DISTRIBUTION? IF SO, NEED TO SCALE EFFECT SIZE
  #for(t in 1:12) {
...
dag <- siml %>% group_by(subjID, trial_type, trial_num) %>%
  tidyboot::tidyboot_mean(looking_time) 

ggplot(dag, aes(x=trial_num, y=looking_time, group=trial_type, color=trial_type)) + 
    geom_point(alpha=.8)
```
...
fit_model <- function(siml) {
  ...
  return(summary(m_simple)$coefficients["trial_typesame",])
}
```

```{r, power-analysis}
fixef(m_simple)["trial_typesame"] <- effect_size
fixef(m_simple)["trial_num"] <- 0.1
p <- powerSim(m_simple, test = fixed("trial_typesame"), nsim =100) 
#we need to specify the fixed effect that we want to do, cause simrR can only calculate power for one particular coefficient each time
```

## Other approaches

```{r}
# https://www.rdocumentation.org/packages/lme4/versions/1.1-23/topics/simulate.merMod
mmod <- simulate(m_simple, nsim = 10, seed = 123)
# this generates new data from the lmer model...better to use this or our own custom function?
```
...
```{r}
#model2 <- extend(m_simple, along="subjID", n=360)
#pc <- powerCurve(model2) 
#print(pc)

model1 <- glmer(z ~ x + (1|g), family="poisson", data=simdata)
fixef(model1)
powerSim(model1) # 82% power, but lots of singularities...
```
